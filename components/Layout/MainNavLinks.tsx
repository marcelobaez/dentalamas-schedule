...
import { IconCalendarEvent, IconClock2, IconUsers } from '@tabler/icons';
    ...
    <>
    {
      links.map((item, idx) => (
        <Link key={`nav-link-${idx}`} href={item.link} passHref>
          <NavLink component="a" onClick={() => setOpened(false)} icon={item.icon} label={item.label} active={router.pathname === item.link} />
        </Link>
      ))
    }
    </>
